...
SWEP.Subsonic 				= false
SWEP.WeaponSkin				= "weapon_none"
SWEP.BuiltInScope			= false
...
function SWEP:AttachmentCheck()
	...
	if IsValid( self.Owner ) then
		...
		local sight = self.VElements.rds and true or self.VElements.m145 and true or self.VElements.scope and true or self.BuiltInScope and true or false
		local pos, ang = 
...
function SWEP:SetIron(sight, pos, ang)
	...
	self.Zoom = self.VElements.m145 and 50 or self.VElements.scope and 30 or self.BuiltInScope and 30 or self.DZoom
...
function SWEP:FOV()
	...
	if self:GetIronsights() and !self.ADS and self.Owner.Zoomed then
		if SERVER then 
			//PrintTable(self.Zoom)
			self.Owner:SetFOV(self.Zoom , 0.2) 
		end
		self.ADS = true
	end
	...
	if self:GetIronsights() and !self.ADS and self.Owner.Zoomed then
		...
		local scopezoom = self.Zoom
	
		if self.UseScope then
	
			scopezoom = self.Owner.SZoomed and self.Zoom*0.6 or self.Zoom
	
		end
		
		if SERVER then
			 self.Owner:SetFOV(scopezoom , 0.2)
		end
...
function SWEP:RunAnglePreset() --Preset run angles for rifles/pistols/retardedbackwardsmodeledbullshitbecausewhoevermodeledthecssweaponsisfuckingretardedandshoulddie.
	if self.ModelRunAnglePreset == 0 then
		...
		self.RunArmOffset = Vector(1,7,5)
	end
	if self.ModelRunAnglePreset == 1 then //ak47's running angle
		self.RunArmAngle  = Angle(-10, -50, -6)
		self.RunArmOffset = Vector(1,7,12)
	end
	if self.ModelRunAnglePreset == 2 then //famas's running angle
		self.RunArmAngle  = Angle(-10, -50, -6)
		self.RunArmOffset = Vector(1,5,12)
	end
	if self.ModelRunAnglePreset == 3 then
		self.RunArmAngle  = Angle(-10, 0, 0)
	...
	if self.ModelRunAnglePreset == 5 then //Legacy CSS Weapons
...
function SWEP:GetViewModelPosition( pos, ang )
	...
	/*Bolt Movement*/
		if self.BoltBone then
			...
			local Slide = VM:LookupBone(self.BoltBone)
...
function SWEP:Crosshair()
	...
	if self.RedDotSight and self:GetIronsights() then
		...
		surface.DrawTexturedRect( x-(ZF*32-MD*2)/2, y-(ZF*32-MD*2)/2+(scale*30), ZF*32-MD*2, ZF*32-MD*2)
		surface.SetDrawColor(math.Rand(200,255),150,150,255)
		surface.SetMaterial(self.RDot2)
		surface.DrawTexturedRect( x-(ZF*16-MD*2)/2, y-(ZF*16-MD*2)/2+(scale*30), ZF*16-MD*2, ZF*16-MD*2)
