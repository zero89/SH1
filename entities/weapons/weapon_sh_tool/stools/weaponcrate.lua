...
if SERVER then
	function MakeWeaponCrate( ply, pos, ang )
		...
		if ply:HasVIP() then
			...
			if(ply:IsBronzeVIP()) then
				maxCrates = 2
			end
			
			if(ply:IsSilverVIP()) then
				maxCrates = 3
			end
			...
			if(ply:IsPlatinumVIP()) then
				maxCrates = 5
			end
		
		end
	
		if ply:GetCount( "sent_weaponcrate" ) >= maxCrates then 
			ply:SendMessage("You already have a crate elsewhere.") 
			return false 
		end
...
elseif CLIENT then
	function TOOL:DoGhostEntity( pos, ang )
		...
		if !IsValid( self.GhostEntity ) then
			self.GhostEntity = ClientsideModel( "models/Items/ammocrate_smg2.mdl" )
