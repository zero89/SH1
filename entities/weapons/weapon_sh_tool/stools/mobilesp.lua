...
if CLIENT then
	...
	language.Add( "Undone_base marker", "Undone Base Marker" )
end
...
function TOOL:RightClick( trace )
	...
	if CLIENT then return true end
	local ply = self:GetOwner()
	local model = "models/props_combine/combine_light001a.mdl"
	local Ang = trace.HitNormal:Angle()
	Ang.pitch = Ang.pitch + 90

	local mbm = MakeBaseMarker( ply, Ang, trace.HitPos+Vector(0,0,3), model )
	if IsValid( mbm ) then
		mbm:SetMaxHealth( 2500 )
		mbm:SetHealth( 500 )
...
if SERVER then
	...
	function MakeBaseMarker( ply, Ang, Pos, Model )
		if ply:GetCount( "markers" ) > 0 then ply:SendMessage("You already have a Base Marker.") return false end
...
elseif CLIENT then
	function TOOL:DoGhostEntity( pos, ang )
		...
		local model = self.SWEP:GetFireMode() == 1 and "models/props_combine/combine_light001a.mdl" or "models/props_combine/combine_mine01.mdl"
