...
if SERVER then
	...
	function GM:PlayerSpawnProp( ply, model )
		...
		if(table.HasValue(self.SpawnLists["VIP"], string.lower(model))) then
		
			if(ply:HasVIP()) then
				return true
			else
				chat.AddText(ply, Color(0, 255, 0), "Sorry! These props are for VIP subscribers only!")
				return false
			end
