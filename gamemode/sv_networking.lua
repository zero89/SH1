...
util.AddNetworkString("sh_sell_item")
...
net.Receive("sh_sell_item", function(len, ply)

	if not IsValid(ply) then return end

	local itmType 	= net.ReadInt(16)
	local itemName 	= net.ReadString()
	local license	= ply:GetLicense(itmType, itemName)
	local cost		= 0
	
	if(license == nil) then
		ply:SendLua([[Derma_Message("You do not own this item", "Error")]])
		return
	end
	
	if(license > -1) then
		ply:SendLua([[Derma_Message("You cannot sell rented weapons", "Error")]])
		return
	end

	if(itmType == 1) then
		cost = tonumber(GAMEMODE.PrimaryWeapons[itemName].price)
	elseif(itmType == 2) then
		cost = tonumber(GAMEMODE.SecondaryWeapons[itemName].price)
	else
		ply:SendLua([[Derma_Message("Invalid weapon type.", "Error")]])
		return 
	end
	
	if(cost == 0) then
		ply:SendLua([[Derma_Message("Unable to get Price data.", "Error")]])
		return
	end
	
	ply:RemoveLicense(itmType, itemName)
	
	ply:ModifyMoney(cost, true)
	ply:AddScoreEvent("Item Sold", cost, true, false, true, "$OW")
	
	ply:SendLua([[Derma_Message("Item has been sold!", "Item Sold")]])
