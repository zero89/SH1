...
function GM.Huds.Slim:DrawTimer( GAMEMODE )
	...
	local tw_title, _ = surface.GetTextSize( str_title )
	
	surface.SetFont( "gbux_defaultbold" )
	local tw_countdown, _ = surface.GetTextSize( str_countdown )
	
	-- Draw background
	local sw = ScrW()
	local w, h = tw_title + tw_countdown + 20, 21
	local x = sw - w
	surface.SetTexture( TEX_GRADIENT_RIGHT )
	surface.SetDrawColor( 20, 20, 20, 200 )
	surface.DrawTexturedRect( x, 0, w, h )
	surface.SetDrawColor( 200, 200, 200, 200 )
	surface.DrawTexturedRect( x, h-2, w, 1 )
	
	-- Draw text
	surface.SetTextColor( 255, 255, 255, 220 )
	surface.SetTextPos( sw-w+15+tw_title, 4 )
	surface.DrawText( str_countdown )
	surface.SetTextPos( sw-w+15, 1 )
	surface.SetFont( "Trebuchet19" )
	surface.DrawText( str_title )
end
local difference = 0
